...
{
    public class LoopValidator : IPuzzleValidator
    {
        ...
        public LoopValidator(LevelConfig config)
        {
        ...
        public bool IsSlotCorrect(int slotIndex)
        {
            ...
            // Check against expected solution (which encodes the full expanded loop)
            if (_slotOccupancy[slotIndex] != _config.expectedSolution[slotIndex])
                return false;

            // Also verify loop pattern consistency: slot N should match slot N % loopBodyLength
            var bodyLen = _config.loopBodyLength > 0 ? _config.loopBodyLength : _config.SlotCount;
            var bodyIndex = slotIndex % bodyLen;
            if (bodyIndex != slotIndex && _slotFilled[bodyIndex])
            {
                // Repeated slot must match body slot value
                return _slotOccupancy[slotIndex] == _slotOccupancy[bodyIndex];
            }
            return true;
        }
